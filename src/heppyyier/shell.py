...
def write_tcl_modulefile(name: str, version: str, prefix: pathlib.Path) -> pathlib.Path:
    ...
    lines = [
        ...
        f"setenv HEPPYYIER_LOADED_{NAME} {version}",
        f"setenv HEPPYYIER_LOADED_{NAME}_PREFIX $prefix",
    ]
    if (p / "bin").is_dir():
...
# Auto-generated by heppyyier. Loads packages whose HEPPYYIER_LOADED_* env var is set.
# Run `heyy generate-modules` to refresh. Do not edit manually.
import os as _os

def _heppyyier_autoload():
    if not any(k.startswith("HEPPYYIER_LOADED_") for k in _os.environ):
        return
    try:
...
def write_sitecustomize() -> pathlib.Path:
    """Write sitecustomize.py to the active venv's site-packages.

    When present, Python runs this file at startup. If any HEPPYYIER_LOADED_*
    env vars are set (e.g. by 'module load fastjet'), the corresponding packages
    are loaded automatically so 'import fastjet' works without explicit
    heppyyier.load() calls.
    """
    import sysconfig
    site_dir = pathlib.Path(sysconfig.get_paths()["purelib"])
    site_dir.mkdir(parents=True, exist_ok=True)
    dest = site_dir / "sitecustomize.py"
    dest.write_text(_SITECUSTOMIZE_TEMPLATE)
    return dest
