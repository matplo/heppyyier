...
def write_sitecustomize() -> pathlib.Path:
    """Write heppyyier_autoload.pth to the active venv's site-packages.
    ...
    The .pth file imports heppyyier._autoload which calls heppyyier.load() for
    every package whose HEPPYYIER_LOADED_* env var is set by 'module load'.
    """
    import sysconfig
    ...
    if old.exists() and "heppyyier" in old.read_text():
        old.unlink()

    dest = site_dir / "heppyyier_autoload.pth"
    dest.write_text("import heppyyier._autoload\n")
    return dest
